...
export default function CreateVotingPage() {
    ...
    const validateVotingData = () => {
        ...
        if (candidates.length === 0) {
            errors.push("Добавьте хотя бы одну опцию.");
        }
        if (voters.length === 0) {
    ...
    return (
        <>
            <div className="flex justify-evenly items-stretch h-screen">
                ...
                {/* 📦 Правая часть */}
                <div className="flex-1 m-20 bg-secondary rounded-xl shadow-lg flex flex-col">
                    <h2 className="text-2xl text-center font-bold mt-5 mb-4">Голосующие</h2>
                    ...
                    <div className="flex px-6 mb-4">
                        <input
                            ...
                            placeholder="Введите I.C. голосующего"
                            className="flex-grow p-2 rounded-l-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary"
                        />
                        <button
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
                ...
                <div className="flex gap-4 mb-4">
                    ...
                    <div className="flex-1 bg-gray-100 rounded-lg p-3 max-h-[30ch] overflow-y-auto">
                        <p className="font-medium mb-2">Опции:</p>
                        <ol className="list-decimal list-inside text-gray-700">
                            {candidates.length ? (
                                candidates.map((c, i) => <li key={i}>{c}</li>)
                            ) : (
                                <li className="italic text-gray-400">Нет опций</li>
                            )}
                        </ol>
                    </div>
